...
router.post("/", async (req, res) => {
  try {
    ...
    const { 
      title,
      description, 
      image, // Added the missing image field
      targetAmount, 
      equity, 
      minimumInvestment,
      businessPlan,
    ...
    const campaign = await prisma.campaign.create({
      data: {
        ...
        title,
        description,
        //image: image || "", // Provide a default empty string if image is not provided
        targetAmt: targetAmount ? parseInt(targetAmount, 10) : 0,
        equityOffered: equity ? parseFloat(equity) : 0,
        minInvestmentAmt: minimumInvestment ? parseInt(minimumInvestment, 10) : 0,
        buisnessPlan: businessPlan || "",
        ...
        startDate,
        endDate,
        organizationId:"1", // Assuming a default organization ID for now
        isActive: true
      },
    });
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const { 
      title, 
      description,
      image, // Added the missing image field here too
      targetAmount, 
      equity, 
      minimumInvestment,
      businessPlan,
